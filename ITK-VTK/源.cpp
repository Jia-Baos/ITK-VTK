...
#include "itkRegularStepGradientDescentOptimizerv4.h"   //常规梯度下降优化器头文件
...
main(int argc, char* argv[])
{
    ...
    // Software Guide : BeginCodeSnippet
    // 将读取的 fixed image 和 moving image 送入 registration
    ...
    // Software Guide : BeginCodeSnippet
    // 为 fixed image 设置初始变换（单位矩阵，相当于无变换）
    ...
    // Software Guide : BeginCodeSnippet
    // 获取 moving image 到 fixed image 的最终变换
    ...
    //  Section~??? for details on the use of this
    ...
    // 整前面设置的resampler的补的值维1，避免对补的值太大导致剪影图像结果动态范围受到影响
    ...
    // Software Guide : BeginCodeSnippet
    // 使用一个新的 writer 保存灰度变换之后的结果
    auto writer2 = WriterType::New();
    writer2->SetInput(intensityRescaler->GetOutput());
    // Software Guide : EndCodeSnippet

    writer2->SetFileName(Args[3]);
    writer2->Update();
    ...
    //  Figure~???.  The components of the
    //  registration method are depicted as well.  Figure
    //  ??? (left) shows the result of resampling
    ...
    //  optimization problem. Figure ??? helps to
    ...
    //  is shown in section~???. This method can
